...
from utils.utils import PHASE, CONF_OPT, BUTTON, FT_SMALL


class ConfigScreen(TemplateScreen):
    """Écran de configuration gérant les paramètres de plongée (gaz, alarme, compas, BLE)
    et la navigation entre les phases de sélection.
    """

    def __init__(self):
        """Initialise l'écran de configuration et les champs visuels."""
        super().__init__()
        # Initiate Attributs
    ...
    def up_phase(self):
        """Fait remonter la phase de sélection"""
    ...
    def down_phase(self):
        """Fait descendre la phase de sélection"""
        if self.phase == PHASE.SELECT_O_ALARME:
    ...
    def update(self, button):
        """Traite une entrée utilisateur et met à jour.

        @param button:  bouton pressé.
        @return: CONF_OPT.CMP_CL | CONF_OPT.BLE_CN | True | None —
                 peut retourner une constante CONF_OPT (pour calibration/BLE),
                 True si la commande de démarrage est demandée, ou None sinon.
        """
        match button:
            case BUTTON.BACK_BUTTON:
                if (
                    self.phase is PHASE.SELECT_GAZ_PER
                    or self.phase is PHASE.SELECT_ALARME
                    or self.phase is PHASE.SELECT_GAZ
                ):
                    self.phase = self.phase.previous()

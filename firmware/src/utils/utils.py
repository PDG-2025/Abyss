...
HALF_TIMES_N2 = [4.0, 5.0, 8.0, 12.5, 18.5, 27.0, 38.3, 54.3,
                 77.0, 109.0, 146.0, 187.0, 239.0, 305.0, 390.0, 498.0]
HALF_TIMES_HE = [1.51, 1.88, 3.02, 4.72, 6.99, 10.21, 14.48, 20.53,
                 29.11, 41.20, 55.19, 70.69, 90.34, 115.29, 147.42, 188.24]
# Coefficients ZHL-16C (a, b) pour N2
A_N2 = [1.1696, 1.0000, 0.8618, 0.7562, 0.6667, 0.5933, 0.5282, 0.4701,
...
A_HE = [1.6189, 1.3830, 1.1919, 1.0458, 0.9220, 0.8205, 0.7305, 0.6502,
        0.5950, 0.5545, 0.5333, 0.5189, 0.5181, 0.5176, 0.5172, 0.5119]
B_HE = [0.4770, 0.5747, 0.6527, 0.7223, 0.7582, 0.7957, 0.8279, 0.8553,
...
def buehlmann_zhl16c_ndl_palier(profile: List[Dict],
        gaz: Dict[str, float],
        gf_low: float = 0.3,
        gf_high: float = 0.85,
...
def ndl_palier_tpalier(o2):
    ...
    return buehlmann_zhl16c_ndl_palier(
        profile=profile,
        gaz={"O2": o2, "N2": 100-o2, "He": 0.0},  # Air
    )

def up_down_from_file():
    """
    Lit un fichier JSON contenant une liste de mesures,
    calcule la vitesse verticale (m/min) sur les 5 dernières.
    Retourne une string formatée '↑ 00' ou '↓ 00'.
    """
    with LOCK_JSON:
    ...
    times = [datetime.fromisoformat(d["timestamp"].replace("Z", "+00:00")) for d in dernieres]
    profondeurs = [d["profondeur_m"] for d in dernieres]

    delta_profondeur = profondeurs[-1] - profondeurs[0]  # m
    delta_temps = (times[-1] - times[0]).total_seconds() / 60  # minutes
    ...
    # Déterminer le sens et formatter
    if vitesse > 0:

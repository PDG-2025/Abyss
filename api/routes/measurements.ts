...
export const measurementsBulkRouter = Router(); // POST /measurements/...
export const divesMeasurementsRouter = Router({ mergeParams: true }); // GET /dives/:dive_id/measurements

measurementsBulkRouter.post(
  "/bulk/:dive_id",
  requireAuth,
...
divesMeasurementsRouter.get(
  "/:dive_id/measurements",
  requireAuth,
  async (req, res, next) => {
    try {
      const uid = (req as any).user.user_id;
      const dive_id = Number((req as any).params.dive_id);
      if (Number.isNaN(dive_id))
        return res.status(400).json({ error: "Invalid dive_id" });
      ...
      const p = Math.max(1, Number((req.query as any).page || 1));
      const lRaw = Number((req.query as any).limit || 500);
      const l = Math.min(Math.max(1, lRaw), 2000);
      const offset = (p - 1) * l;
      ...
      const sql = `SELECT timestamp, depth_current, temperature, ascent_speed, air_pressure, cumulative_ascent
                 FROM "Measurement"

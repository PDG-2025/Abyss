...
export async function listDives(params: {
  page?: number;
  limit?: number;
  from?: string;
  to?: string;
...
export async function updateDive(diveId: number, payload: any) {
  const res = await api.patch(`/dives/${diveId}`, payload);
  return res.data;
}

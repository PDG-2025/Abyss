...
type DiveItem = { dive_id: number; title?: string | null; duration: number; depth_max: number; date: string };
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const maxDepth = items.reduce((m, d) => Math.max(m, Number(d.depth_max || 0)), 0);
        const totalMin = (r as any).aggregate_minutes ?? items.reduce((s, d) => s + Number(d.duration || 0), 0);
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: palette.bg }} contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
      <Text style={{ color: palette.text, textAlign: 'center', fontWeight: '700', marginBottom: 16 }}>Home</Text>

      <View style={{ alignItems: 'center', marginBottom: 16 }}>
        <Image
          source={{ uri: `https://api.dicebear.com/7.x/avataaars/png?seed=${encodeURIComponent(user?.name || 'Abyss')}` }}
          style={{ width: 96, height: 96, borderRadius: 48, backgroundColor: palette.card }}
        />
        <Text style={{ color: palette.text, fontSize: 20, fontWeight: '700', marginTop: 8 }}>{user?.name || '—'}</Text>
        <Text style={{ color: palette.sub, fontSize: 13 }}>Divemaster</Text>
      </View>

      <View style={{ flexDirection: 'row', gap: 12 }}>
        <View style={{ flex: 1, backgroundColor: palette.card, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: palette.border }}>
          <Text style={{ color: palette.sub, marginBottom: 6 }}>Total Dives</Text>
          <Text style={{ color: palette.text, fontSize: 24, fontWeight: '800' }}>{stats.total}</Text>
        </View>
        <View style={{ flex: 1, backgroundColor: palette.card, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: palette.border }}>
          <Text style={{ color: palette.sub, marginBottom: 6 }}>Max Depth</Text>
          <Text style={{ color: palette.text, fontSize: 24, fontWeight: '800' }}>{stats.maxDepth}m</Text>
        </View>
      </View>

      <View style={{ backgroundColor: palette.card, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: palette.border, marginTop: 12 }}>
        <Text style={{ color: palette.sub, marginBottom: 6 }}>Total Time</Text>
        <Text style={{ color: palette.text, fontSize: 24, fontWeight: '800' }}>{totalTimeLabel}</Text>
      </View>

      <Text style={{ color: palette.text, fontSize: 18, fontWeight: '700', marginTop: 20, marginBottom: 10 }}>Recent Dives</Text>
      {dives.map((d) => (
        <TouchableOpacity
          key={d.dive_id}
          onPress={() => navigation.navigate('DiveDetail', { dive_id: d.dive_id })}
          style={{
            flexDirection: 'row',
            alignItems: 'center',
        ...
        >
          <Image source={{ uri: coverByDive[d.dive_id] || PLACEHOLDER }} style={{ width: 84, height: 60, borderRadius: 8, backgroundColor: palette.badge }} />

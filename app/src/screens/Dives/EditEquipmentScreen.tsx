...
export default function EditEquipmentScreen() {
  ...
  const [existingEquipment, setExistingEquipment] = useState<any>(null);

  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const eq = await getEquipment(dive_id);
        setExistingEquipment(eq);

        if (eq) {
          setWetsuitThickness(
            eq.wetsuit_thickness != null ? String(eq.wetsuit_thickness) : ""
          );
          setTankSize(eq.tank_size != null ? String(eq.tank_size) : "");
          setTankPressureStart(
            eq.tank_pressure_start != null ? String(eq.tank_pressure_start) : ""
          );
          setTankPressureEnd(
            eq.tank_pressure_end != null ? String(eq.tank_pressure_end) : ""
          );
          setWeightsUsed(
            eq.weights_used != null ? String(eq.weights_used) : ""
          );
        } else {
          setWetsuitThickness("");
          setTankSize("");
  ...
  const onSave = async () => {
    try {
      const payload = {
        wetsuit_thickness:
          wetsuit_thickness !== ""
            ? Number(wetsuit_thickness)
            : existingEquipment?.wetsuit_thickness ?? null,
        tank_size:
          tank_size !== ""
            ? Number(tank_size)
            : existingEquipment?.tank_size ?? null,
        tank_pressure_start:
          tank_pressure_start !== ""
            ? Number(tank_pressure_start)
            : existingEquipment?.tank_pressure_start ?? null,
        tank_pressure_end:
          tank_pressure_end !== ""
            ? Number(tank_pressure_end)
            : existingEquipment?.tank_pressure_end ?? null,
        weights_used:
          weights_used !== ""
            ? Number(weights_used)
            : existingEquipment?.weights_used ?? null,
      };

      await upsertEquipment(dive_id, payload);
  ...
  return (
    ...
    >
      ...
      <Text style={{ color: palette.sub, marginBottom: 4 }}>
        Épaisseur combinaison (mm)
      </Text>
      <TextInput
        value={wetsuit_thickness}
        onChangeText={setWetsuitThickness}
      ...
      <Text style={{ color: palette.sub, marginBottom: 4 }}>
        Pression départ (psi/bar)
      </Text>
      <TextInput
        value={tank_pressure_start}
        onChangeText={setTankPressureStart}
        keyboardType="numeric"
        placeholder="200"
        placeholderTextColor={palette.hint}
        style={inputStyle}
      ...
      <Text style={{ color: palette.sub, marginBottom: 4 }}>
        Pression fin (psi/bar)
      </Text>
      <TextInput
        value={tank_pressure_end}
        onChangeText={setTankPressureEnd}
        keyboardType="numeric"
        placeholder="50"
        placeholderTextColor={palette.hint}
        style={inputStyle}
